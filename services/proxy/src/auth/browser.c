...
#define FIREFOX_DIR_SNAP  "%s/snap/firefox/common/.mozilla/firefox"
#define FIREFOX_DIR_STD   "%s/.mozilla/firefox"
...
int find_firefox_cookies(char *out, size_t out_size)
{
    ...
    const char *patterns[] = { FIREFOX_DIR_SNAP, FIREFOX_DIR_STD };
    int found = 0;
    time_t newest = 0;

    for (int p = 0; p < 2; p++) {
        snprintf(base, sizeof(base), patterns[p], home);
...
{
    char tmp[64];
    snprintf(tmp, sizeof(tmp), "/tmp/uva_cookies_%d.sqlite", getpid());
